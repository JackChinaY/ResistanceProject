 ...
 * call(对象名，操作名)方法：调用COM对象的方法，返回Variant类型值。
 * invoke( )方法：和call方法作用相同，但是不返回值。
 * get(对象名，属性)方法：获取COM对象属性，返回variant类型值。
 * put(对象名，属性，属性值)方法：设置COM对象属性。
 */
...
public class WordManager {
     ...
     * word运行程序对象
     */
    private ActiveXComponent word; // 创建office的一个应用，比如你操作的是word还是excel
    ...
    public WordManager(boolean visible) {
        if (word == null) {
            word = new ActiveXComponent("Word.Application"); //  初始化word应用程序
            word.setProperty("Visible", new Variant(visible)); // 配置启动word时是显示执行还是隐式执行  true为显示执行  false为隐式
        }
        if (documents == null)
    ...
    public void openDocument(String docPath) {
        closeDocument();
        doc = Dispatch.call(documents, "Open", docPath).toDispatch();// docPath要打开的文档的详细地址
        selection = Dispatch.get(word, "Selection").toDispatch();// 获得该文档对象，并返回
    }
    ...
    public boolean find(String toFindText) {
        if (toFindText == null || toFindText.equals(""))
            return false;
        // 从selection所在位置开始查询
        Dispatch find = Dispatch.call(selection, "Find").toDispatch(); // 打开word自带的搜索功能,selection为要执行操作的对象，Find为要执行的操作名称
        // 设置要查找的内容
        Dispatch.put(find, "Text", toFindText);// 设置find对象Text属性，具体参数为toFindText
        // 向前查找
        ...
        // 全字匹配
//		Dispatch.put(find, "MatchByte", "True");
        // 查找并选中
...
//	}
     ...
     * @param newText    要替换的内容
     */
    public void setHeader(String toFindText, String newText) {
     ...
     * 全局替换图片
     *
     * @param toFindText 查找字符串 （第一个参数）
     * @param imagePath  图片路径 （第二个参数）
     */
